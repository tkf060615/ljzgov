          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
             {displayItems.map((item, idx) => (
                 <div key={item.id} onClick={() => onNavigate && onNavigate(item.id)} className="group cursor-pointer">
                    <div className="solid-card p-8 rounded-lg group-hover:shadow-card-hover group-hover:border-brand-accent/50 transition-all bg-white dark:bg-slate-800 h-full flex flex-col justify-between border border-transparent">
                        <div>
                            ...
                            <h3 className="text-2xl font-bold text-brand-dark dark:text-gray-100 leading-snug group-hover:text-brand-accent transition-colors mb-4">
                                {item.title}
                            </h3>
                        ...
                        <div className="mt-6 flex justify-end">
                             <ArrowRight size={20} className="text-gray-300 group-hover:text-brand-accent transition-colors" />
                        </div>
                    </div>
                 </div>
             ))}
