...
const Navbar: React.FC<NavbarProps> = ({ currentSection, onNavigate, currentLang, onLanguageChange, theme, onToggleTheme, onSearch }) => {
  ...
  return (
    <nav className={`fixed left-0 right-0 z-50 transition-all duration-300 ease-in-out ${navClasses}`}>
      <div className="container mx-auto px-4 lg:px-6">
        <div className="flex justify-between items-center">
          ...
          >
            ...
            <div className={`flex flex-col ${textClasses}`}>
              <span className="text-lg font-bold tracking-tight leading-none uppercase font-sans">
                {currentLang === 'ZH' ? '隆家寨' : 'Longjiazhai'}
              </span>
              <span className="text-[10px] tracking-[0.2em] uppercase opacity-80 mt-1">
                {currentLang === 'ZH' ? '实验区人民政府' : 'Experiment Zone Gov'}
              </span>
            </div>
          </div>
          ...
          <div className="hidden md:flex items-center space-x-1">
                ...
                <button 
                    onClick={() => onNavigate(SectionType.MAYOR)}
          ...
          <div className="md:hidden flex items-center gap-2">
            ...
            <button
              ...
              className={`p-2 rounded-lg ${textClasses} ${buttonHoverClass} transition-transform duration-300 ${isMobileMenuOpen ? 'rotate-90' : 'rotate-0'}`}
            >
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden absolute top-full left-0 right-0 bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl border-b border-gray-200 dark:border-gray-800 shadow-xl animate-slide-down origin-top">
          <div className="flex flex-col p-6 space-y-2">
            ...
            {navItems.map((item) => (
              <button
                ...
                className="w-full text-left text-lg font-medium text-slate-800 dark:text-white hover:text-brand-accent transition-all py-3 border-b border-gray-100 dark:border-gray-800"
              >
                {item.label}
              </button>
            ))}
